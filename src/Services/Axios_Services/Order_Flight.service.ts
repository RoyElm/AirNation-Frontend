...
import { deletedOrderFlightAction, OrderFlightAddedAction, OrderFlightsDownloadedAction } from '../../Redux/OrderFlightsState';
...
export async function deleteOrderFlightAsync(_id: string): Promise<void> {
    await axios.delete(Globals.orderFlightApiUrl + _id);
    store.dispatch(deletedOrderFlightAction(_id));
}
